...
cannot serve the request (never charged). A small flat discovery fee applies —
it buys the ranking itself (price × latency × verified on-chain reputation over
the whole catalog). Managed keys pay from balance; BYO wallets call once without
`payment` to receive the x402 signing quote, then re-call with the signed
authorization.

```ts
const found = await client.callTool('discover', { request: 'technical SEO audit of a web page' });
// → { type: 'candidates', recommended: '…', shortlist: [...], discovery_fee_usdc: … }
